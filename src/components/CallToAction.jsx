 ...
 */
const CallToAction = ({ className = '', isEmbedded = false }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.dataSystems.trim()) {
      newErrors.dataSystems = 'Data systems information is required';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        body: JSON.stringify({
          ...
          dataSystems: formData.dataSystems
        })
      });
      ...
      setIsSuccess(true);
      setFormData({ email: '', companyName: '', dataSystems: '' });
  ...
  // If embedded, return just the content without the section wrapper
  if (isEmbedded) {
    return isSuccess ? successContent : (
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Data Systems Input */}
        <div className="text-left">
          <label htmlFor="dataSystems" className="block text-sm font-medium text-iron-grey mb-2">
            Data Systems <span className="text-takt-orange">*</span>
          </label>
          <textarea
            id="dataSystems"
            name="dataSystems"
            value={formData.dataSystems}
            onChange={handleInputChange}
            rows={3}
            className={`w-full px-4 py-3 border rounded-takt focus:outline-none focus:ring-2 focus:ring-takt-orange focus:border-transparent transition-colors resize-none ${
              errors.dataSystems ? 'border-red-500' : 'border-gray-300'
            }`}
            placeholder="Tell us about your current data systems (ERP, MES, SCADA, etc.)"
            disabled={isSubmitting}
          />
          {errors.dataSystems && (
            <p className="text-red-500 text-sm mt-1">{errors.dataSystems}</p>
          )}
        </div>
  ...
  // Full section version with background for standalone use
  if (isSuccess) {
    return (
        ...
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-white/95 backdrop-blur-sm rounded-takt shadow-xl p-8 md:p-10">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-takt-green mb-8">
              Tell us about your needs, <span className="text-takt-orange">what data systems do you use?</span>
            </h2>
